...
/* 
 * Exiba o endereço, a data de entrada e de saída dos estacionamentos 
 * do veículo de placa "YZA4567":
*/

select p.endereco, e.dataentrada, e.datasaida
from estaciona e
join patio p on e.patio_id = p.id
join veiculo v on v.id = e.veiculo_id
where v.placa = 'YZA4567';
...
from estaciona
join veiculo on estaciona.veiculo_id = veiculo.id
join modelo on modelo.id = veiculo.modelo_id
where estaciona.num = 2;

/*
* Exiba a placa, o nome dos donos e a descrição dos modelos de todos os veículos:
*/

select cliente.nome as cliente, modelo.descricao as Modelo, veiculo.placa  
from veiculo
join cliente on veiculo.cliente_id = cliente.id
join modelo on modelo.id = veiculo.modelo_id
order by cliente, modelo;

-- Exiba o número total de veículos cadastrados no sistema:

select COUNT(*) from veiculo;

-- Calcule a soma das capacidades de todos os pátios.

SELECT SUM(capacidade) AS soma_capacidade FROM Patio;

-- Informe a quantidade de veículos estacionados no momento atual;
select count(*) from estaciona where horarioSaida is null;





-- Calcule a média de anos dos modelos de veículos.
SELECT ROUND(AVG(ano)) AS media_anos
FROM Modelo;


-- Qual foi o primeiro dia em que um automovel foi estacionado
SELECT MIN(dataEntrada) AS primeiro_dia_estacionamento
FROM Estaciona;


-- Qual foi o ultimo dia em que um automovel foi estacionado no patio 2
SELECT MAX(dataEntrada) AS ultimo_dia_estacionamento_patio_2
FROM Estaciona
WHERE patio_id = 2;


-- Calcule a quantidade de veículos estacionados em cada pátio
SELECT Patio.num, COUNT(Estaciona.veiculo_id) AS quantidade_veiculos
FROM Patio
LEFT JOIN Estaciona ON Patio.id = Estaciona.patio_id
GROUP BY Patio.num;
